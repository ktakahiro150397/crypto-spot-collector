...
- routing一致後のタスクについては、継続的許可により正規化、このRepository内で必要な分割、子タスク作成、既存labelを保持したlabel設定、優先度見直し、親子のセクション移動、対応コメント追加を行ってよい。別Repositoryの成果は作成・変更せず報告する。削除・完了は含まない。
- 分割した追跡親には`保留`labelを付け、未完了の実行可能な子がある間は自動選定しない。最初の未ブロック子だけを`次にやる`へ置き、後続を`候補`へ置く。
- 自動選定はcanonical repository labelがあり、他のrepository labelと競合しないタスクだけを対象とする。`次にやる`を`候補`より優先し、`p1`、`p2`、`p3`、同順位では古い順とし、最大1件を選ぶ。`p4`、`保留`、未完了子を持つ追跡親、他の実行が着手済みのタスクは除外する。`p4`はユーザーが対象を明示した場合だけ着手する。
