...
def should_long(df: pd.DataFrame) -> bool:
    ...
    Returns:
        ...
        False: ロングシグナルなし
    """
    # TODO: 実際のロング判断ロジックを実装する
    # 例: RSIが30以下から上昇、MACDがゴールデンクロスなど
    logger.debug("should_long: プレースホルダ - 常にFalseを返す")
    return False
...
def should_short(df: pd.DataFrame) -> bool:
    ...
    Returns:
        ...
        False: ショートシグナルなし
    """
    # TODO: 実際のショート判断ロジックを実装する
    # 例: RSIが70以上から下降、MACDがデッドクロスなど
    logger.debug("should_short: プレースホルダ - 常にFalseを返す")
    return False
...
async def main() -> None:
    ...
    while True:
        ...
        fromDateUtc = toDateUtc - timedelta(hours=1)  # 過去1時間分のデータを取得
