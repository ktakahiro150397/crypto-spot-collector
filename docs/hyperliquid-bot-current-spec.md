...
起動後は 3 分ごとにポジションを同期し、ポジションの `percentage >= 7` と判定するとトレーリングを開始する (`buy_perp.py:451-602`)。最初に SL をローカルで保持した entry へ移し、その後は次の簡易 SAR 方式で更新する。
...
インメモリ manager は既存シンボルの更新時に order id と任意の active flag だけを更新し、side、entry、高値・安値、現在 SL、AF を保持する。そのため追加注文、平均建値変更、ポジション反転を正しく同期できない。再起動時は取引所の TP/SL からトレーリング有効らしさを推測するだけで、高値・安値と AF は復元できない (`exchange/trailingstop/trailingstop_manager.py`)。
